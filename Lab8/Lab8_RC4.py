...
def genKey(l=8):
    '''
    generates a random key of length l bytes (8 by default)
    '''
...
if __name__ == "__main__":
    ...
    crypt_time = time.time()
    cipherText = cipher(plainText, schedule(myKey))
    crypt_time = time.time() - crypt_time
    print(f'Cryption time: {crypt_time}')
    print('Cipher bytes: {}'.format(toBytes(toInts(cipherText))))
    decrypt_time = time.time()
    print('Result after decrypting: {}'.format(
        toStr(cipher(cipherText, schedule(myKey)))))
